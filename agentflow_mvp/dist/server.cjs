...
var import_fs, import_path, import_url, import_meta, ROOT_HINT, SNAPSHOT_PREFIX, isSnapshotPath, extractSnapshotFromWindowsPath, normalizeSnapshotValue, toAbsoluteIfPossible, deriveSnapshotRoots, moduleDir, MODULE_ROOT, DEFAULT_ROOT, EXEC_ROOT, SNAPSHOT_CANONICAL_ROOT, SNAPSHOT_FILESYSTEM_ROOT, SNAPSHOT_ROOT, expandSnapshotCandidate, joinWithRoot, addCandidate, pickWritableRoot, DATA_ROOT, buildAssetRoots, ASSET_ROOTS, DEBUG_PATHS, resolveAssetPath, resolveDataPath;
    import_url = require("url");
    import_meta = {};
    SNAPSHOT_PREFIX = "snapshot:";
    isSnapshotPath = (value) => typeof value === "string" && value.startsWith(SNAPSHOT_PREFIX);
    extractSnapshotFromWindowsPath = (value) => {
    ...
    deriveSnapshotRoots = () => {
      ...
      const canonicalEntry = isSnapshotPath(entry) ? normalizeSnapshotValue(entry) : entry.startsWith("/snapshot") ? normalizeSnapshotValue(`${SNAPSHOT_PREFIX}${entry.slice("/snapshot".length)}`) : fromWindows;
      let filesystemRoot = null;
      if (entry.startsWith("file://")) {
        const fsEntry = (0, import_url.fileURLToPath)(entry);
        filesystemRoot = import_path.default.dirname(fsEntry);
      } else if (!isSnapshotPath(entry) && import_path.default.isAbsolute(entry)) {
        filesystemRoot = import_path.default.dirname(entry);
      }
      const canonicalRoot = canonicalEntry ? import_path.default.posix.dirname(canonicalEntry) : null;
      return {
    ...
    ({ canonical: SNAPSHOT_CANONICAL_ROOT, filesystem: SNAPSHOT_FILESYSTEM_ROOT } = deriveSnapshotRoots());
    SNAPSHOT_ROOT = SNAPSHOT_CANONICAL_ROOT || SNAPSHOT_FILESYSTEM_ROOT || MODULE_ROOT || EXEC_ROOT;
    ...
    buildAssetRoots = () => {
      ...
      if (process.pkg?.entrypoint) {
        addCandidate(roots, import_path.default.dirname(process.pkg.entrypoint));
      }
      if (process.pkg?.defaultEntrypoint) {
        addCandidate(roots, import_path.default.dirname(process.pkg.defaultEntrypoint));
      }
