...
describe('appPaths snapshot handling', () => {
  ...
  test('handles Windows-style snapshot entrypoints', async () => {
    const mountpoint = path.join(tempDir, 'windows-mount');
    const publicDir = path.join(mountpoint, 'dist', 'public');
    fs.mkdirSync(publicDir, { recursive: true });
    const indexPath = path.join(publicDir, 'index.html');
    fs.writeFileSync(indexPath, '<html></html>');

    const windowsEntrypoint = 'C:\\snapshot\\dist\\server.cjs';
    process.pkg = {
      entrypoint: windowsEntrypoint,
      defaultEntrypoint: windowsEntrypoint,
      mountpoint,
    };

    const { resolveAssetPath } = await loadModule();
    const resolved = resolveAssetPath('public', 'index.html');
    expect(resolved).toBe(indexPath);
  });
